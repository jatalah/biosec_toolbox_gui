...
plot_data <- read_rds("combined_plot_small_data.rds")

output$plot <- renderPlot({
  # Ensure the `plot_data` is a list of objects that have `port`, `MNT`, and `plot`
  filtered_plot_data <- plot_data %>%
    keep(~ as.character(.x$port) == input$port_options & as.character(.x$MNT) == input$management_options)
  
  # Check if any plot matches the filter criteria
  if (length(filtered_plot_data) == 0) {
    return(NULL)  # No data available for the selected criteria
  }
  
  # Extract the first matching plot
  selected_plot <- filtered_plot_data[[1]]$plot +
    # Modify the plot to adjust font sizes
    theme(
